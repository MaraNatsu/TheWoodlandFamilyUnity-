...
namespace Assets.SIgnalRServices
{
    class GameProcessor
    {
            ...
            GameObject waitingScreen)
        {
            ...
            SetInstantiationTurn(connectedPlayers, currentPlayer);
            ...
            foreach (var player in connectedPlayers)
            {
                ...
                if (player.Id == currentPlayer.Id)
                {
                    ...
                    Object.Instantiate(healthPoint, gameScreen.transform);
                }
                else
                {
                ...
                instance.GetComponent<Renderer>().material.SetColor("SaturatedYellow", _colorWaiting);
            ...
            {
                case 2:
                    _playerViews.ElementAt(1).Key.GetComponent<RectTransform>().sizeDelta = new Vector3(300, 200);
                    _playerViews.ElementAt(1).Key.transform.localPosition = new Vector3(0, 200);
        ...
        private void SetInstantiationTurn(List<PlayerOutputModel> players, PlayerOutputModel currentPlayer)
        {
            ...
            temp.Add(players.Where(player => player.Turn == currentPlayer.Turn).FirstOrDefault());
        ...
        public void UpdatePlayerViews(PlayerOutputModel updatedPlayer, GameObject healthPoint, GameObject gameScreen)
        {
            ...
            if (_playerViews[key].HealthCount > updatedPlayer.HealthCount)
            {
                Object.Instantiate(healthPoint, gameScreen.transform);
                healthPoint.transform.localPosition = new Vector3(-40, -150);
            }
            else if (_playerViews[key].HealthCount < updatedPlayer.HealthCount)
            {
                Object.Destroy(healthPoint);
            }

            _playerViews[key] = updatedPlayer;
        }

        public void ShowWinner(int winnerId, GameObject winnerText, GameObject gameScreen)
        {
            foreach (var playerView in _playerViews)
            {
                playerView.Key.SetActive(false);
            }

            string winnerName = _playerViews.Values.First(player => player.Id == winnerId).PlayerName;

            gameScreen.SetActive(false);
            winnerText.GetComponentInChildren<Text>().text = "Player " + winnerName + " win!".ToUpper();
